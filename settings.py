...
from backend_status_check import (
    get_backends_with_queue,
...
def get_available_backend():
    ...
    def find_best_backend():
        ...
        list_available_backends_with_queue()  # 목록 출력

        backends_with_queue = get_backends_with_queue()
        if not backends_with_queue:
            raise ValueError("사용 가능한 백엔드가 없습니다.")

        best_backend_name, _ = min(backends_with_queue, key=lambda x: x[1])
        print(f"대기열이 가장 짧은 백엔드를 선택합니다: {best_backend_name}")
        return best_backend_name

    if not backend_name:
        print("환경 변수 'BACKEND'가 설정되지 않았습니다.")
        print("사용 가능한 백엔드 목록과 대기열 정보를 확인합니다.")
        backend_name = find_best_backend()
    ...
    try:
        ...
        if not status.operational:
            print(f"선택된 backend '{backend_name}'는 사용 불가 상태입니다.")
            backend_name = find_best_backend()
            backend = service.backend(backend_name)
